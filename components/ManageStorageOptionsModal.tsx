...
interface ManageStorageOptionsModalProps {
...
export const ManageStorageOptionsModal: React.FC<ManageStorageOptionsModalProps> = ({ storageOptions, setStorageOptions, onClose, addLogEntry }) => {
    ...
    const handleAddItem = () => {
        ...
        if (trimmedName && !storageOptions.find(p => p.toLowerCase() === trimmedName.toLowerCase())) {
            setStorageOptions(prev => [...prev, trimmedName].sort());
            addLogEntry(`Opção de armazenamento de ativo adicionada: "${trimmedName}"`, 'Sistema');
            setNewItemName('');
        }
    };
    ...
    const handleUpdateItem = () => {
        if (editingItem && editingItem.name.trim()) {
            const oldName = storageOptions[editingItem.index];
            const newName = editingItem.name.trim();
            const updatedItems = [...storageOptions];
            updatedItems[editingItem.index] = newName;
            setStorageOptions(updatedItems.sort());
            addLogEntry(`Opção de armazenamento de ativo "${oldName}" renomeada para "${newName}"`, 'Sistema');
            setEditingItem(null);
        }
    };

    const requestDeleteItem = (index: number) => {
        setConfirmingDelete({ index, name: storageOptions[index] });
    };

    const confirmDeleteItem = () => {
        if (confirmingDelete) {
            addLogEntry(`Opção de armazenamento de ativo removida: "${confirmingDelete.name}"`, 'Sistema');
            setStorageOptions(prevOptions => prevOptions.filter((_, index) => index !== confirmingDelete.index));
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
            <Card className="relative w-full max-w-md" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="absolute inset-0 bg-slate-900/70 backdrop-blur-sm z-10 flex items-center justify-center rounded-xl" onClick={() => setConfirmingDelete(null)}>
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-xl max-w-sm w-full mx-4 border border-slate-200/80 dark:border-slate-700/60" onClick={e => e.stopPropagation()}>
                            ...
                            <p className="mt-4 text-sm text-slate-600 dark:text-slate-400">
                                Você tem certeza que deseja excluir a opção de armazenamento <strong className="text-slate-800 dark:text-slate-200">{confirmingDelete.name}</strong>?
                            </p>
                            <div className="mt-4 text-sm bg-red-100 dark:bg-red-500/20 text-red-800 dark:text-red-300 p-3 rounded-md border border-red-200 dark:border-red-500/30">
                                ...
                                <p>Esta ação é irreversível e a opção será removida permanentemente.</p>
                            </div>
                ...
                <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-200 dark:border-slate-700">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-white">Gerenciar Armazenamento</h2>
                ...
                <div className="space-y-4 mb-6">
                    <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300">Adicionar Nova Opção</h3>
                    <div className="flex gap-2 items-end">
                        <div className="flex-grow">
                            <Input
                                label="Descrição do Armazenamento"
                                id="newItem"
                                value={newItemName}
                ...
                <div>
                    <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-2">Opções Existentes</h3>
                    <div className="max-h-60 overflow-y-auto pr-2 -mr-2">
                        <ul className="space-y-2">
                        {storageOptions.length > 0 ? storageOptions.map((item, index) => (
                            <li key={item} className="flex items-center justify-between p-2 rounded-md bg-slate-100 dark:bg-slate-700/50">
                                ...
                                <div className="flex gap-2 ml-4 flex-shrink-0">
                                    {editingItem?.index === index ? (
                                        ...
                                        <>
                                            <Button onClick={() => setEditingItem({ index, name: item })} variant="secondary" className="!p-2" title="Editar Opção">
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                            </Button>
                                            <Button onClick={() => requestDeleteItem(index)} variant="secondary" className="!p-2 !bg-red-500/10 hover:!bg-red-500/20 text-red-500" title="Excluir Opção">
                            ...
                            <p className="text-center text-sm text-slate-500 dark:text-slate-400 py-4">Nenhuma opção cadastrada.</p>
                        )}
                        </ul>
                    </div>
                </div>
