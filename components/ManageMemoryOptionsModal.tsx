...
interface ManageMemoryOptionsModalProps {
...
export const ManageMemoryOptionsModal: React.FC<ManageMemoryOptionsModalProps> = ({ memoryOptions, setMemoryOptions, onClose, addLogEntry }) => {
    ...
    const handleAddItem = () => {
        ...
        if (trimmedName && !memoryOptions.find(p => p.toLowerCase() === trimmedName.toLowerCase())) {
            setMemoryOptions(prev => [...prev, trimmedName].sort());
            addLogEntry(`Opção de memória de ativo adicionada: "${trimmedName}"`, 'Sistema');
            setNewItemName('');
        }
    };
    ...
    const handleUpdateItem = () => {
        if (editingItem && editingItem.name.trim()) {
            const oldName = memoryOptions[editingItem.index];
            const newName = editingItem.name.trim();
            const updatedItems = [...memoryOptions];
            updatedItems[editingItem.index] = newName;
            setMemoryOptions(updatedItems.sort());
            addLogEntry(`Opção de memória de ativo "${oldName}" renomeada para "${newName}"`, 'Sistema');
            setEditingItem(null);
        }
    };

    const requestDeleteItem = (index: number) => {
        setConfirmingDelete({ index, name: memoryOptions[index] });
    };

    const confirmDeleteItem = () => {
        if (confirmingDelete) {
            addLogEntry(`Opção de memória de ativo removida: "${confirmingDelete.name}"`, 'Sistema');
            setMemoryOptions(prevOptions => prevOptions.filter((_, index) => index !== confirmingDelete.index));
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
            <Card className="relative w-full max-w-md" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="absolute inset-0 bg-slate-900/70 backdrop-blur-sm z-10 flex items-center justify-center rounded-xl" onClick={() => setConfirmingDelete(null)}>
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-xl max-w-sm w-full mx-4 border border-slate-200/80 dark:border-slate-700/60" onClick={e => e.stopPropagation()}>
                            ...
                            <p className="mt-4 text-sm text-slate-600 dark:text-slate-400">
                                Você tem certeza que deseja excluir a opção de memória <strong className="text-slate-800 dark:text-slate-200">{confirmingDelete.name}</strong>?
                            </p>
                ...
                <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-200 dark:border-slate-700">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-white">Gerenciar Opções de Memória</h2>
                ...
                <div>
                    ...
                    <div className="max-h-60 overflow-y-auto pr-2 -mr-2">
                        ...
                        {memoryOptions.length > 0 ? memoryOptions.map((item, index) => (
